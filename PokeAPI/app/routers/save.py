...
from ..models.pokemon import Location, Pokemon, Species
...
async def create_save(
    file: UploadFile, session: SessionDep, save_name: str = Form(...)
):
    ...
    if party:
        for poke in party:
            ...
            location_db = Location(
                party=True, slot_number=poke.get('slotNumber')
            )
    ...
    for box in boxes:
        ...
        for poke in pokemons_box:
            location_db = Location(
                party=False, nr_box=nr_box, slot_number=poke.get('slotNumber')
            )
            session.add(location_db)
            session.flush()  # gera location_db.id sem fechar a transação
            name = session.exec(
                select(Species).where(
                    Species.species_id == poke.get('speciesId')
                )
            ).first()
            print(name)
            poke_db = Pokemon(
                **poke, save_id=save_db.id, location_id=location_db.id
            )
            session.add(poke_db)
    session.commit()
